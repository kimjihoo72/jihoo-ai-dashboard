...
# 페이지 설정
st.set_page_config(page_title="지후의 AI 관제 V5.5", layout="wide")
st.title("🛡️ AI 1주일 뒤 목표가 예측 시스템 V5.5")
st.markdown("### 📅 전 그래프 미래 확장: 7일 뒤 시나리오 입체 분석")
...
def run_v55_engine(symbol):
    ...
    # --- [핵심 핵심] 그래프 시각화용 미래 7일 타임라인 강제 확장 데이터프레임 빌드 ---
    last_date = df_clean.index[-1]
    future_dates = [last_date + timedelta(days=i) for i in range(1, 8)]
    
    future_df = pd.DataFrame(index=future_dates, columns=df_clean.columns)
    
    # 미래 시점의 예측 가격 점을 마지막 행에 찍어줌 (선이 이어지도록 출발점과 도착점 세팅)
...
res_dict = {name: run_v55_engine(sym) for name, sym in stocks.items()}
...
for i, (name, res) in enumerate(res_dict.items()):
    ...
    with tabs[i+1]:
        ...
        # 최근 45일 데이터와 미래 7일 데이터를 엮어서 개별 그래프 그리기
        ...
        final_fig.add_bar(x=[f_df.index[-1]], y=[abs(res['pred_price'] - res['price'])], base=[min(res['price'], res['pred_price'])],
                          name="예측 변동폭", marker_color='rgba(255, 161, 90, 0.4)', width=1000*60*60*24*3) # 기둥 두께 확보
